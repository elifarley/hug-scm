...
class TestMainFunction:
    ...
    def test_main_json_output(self, sample_git_log_activity, monkeypatch, capsys):
        ...
        monkeypatch.setattr(
            sys, 'argv',
            ['activity.py', '--by-hour', '--format', 'json']
        )
    ...
    def test_main_text_output(self, sample_git_log_activity, monkeypatch, capsys):
        ...
        monkeypatch.setattr(
            sys, 'argv',
            ['activity.py', '--by-day', '--format', 'text']
        )
...
class TestEdgeCases:
    ...
    def test_handles_all_hours_of_day(self):
        ...
        # Arrange
        log_lines = [
            f"2024-11-17 {h:02d}:00:00 -0500|Alice"
            for h in range(24)
        ]
        log = '\n'.join(log_lines)
        commits = activity.parse_git_log(log)
    ...
    def test_handles_all_days_of_week(self):
        ...
        log_lines = [
            f"2024-11-{18+d} 10:00:00 -0500|Alice"
            for d in range(7)
        ]
        log = '\n'.join(log_lines)
        commits = activity.parse_git_log(log)
...
"""
    ...
    def test_handles_many_authors(self):
        ...
        # Arrange
        log_lines = [
            f"2024-11-17 10:00:00 -0500|Author{i}"
            for i in range(50)
        ]
        log = '\n'.join(log_lines)
        commits = activity.parse_git_log(log)
