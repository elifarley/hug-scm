...
class TestParseArgs:
    ...
    def test_parse_args_validates_format_choices(self, monkeypatch, capsys):
        ...
        # Arrange
        monkeypatch.setattr('sys.argv', [
            'churn.py', 'test.py', '--format', 'invalid'
        ])
...
class TestGetLineHistory:
    ...
    def test_get_line_history_handles_subprocess_exception(self, mock_file, mock_subprocess, capsys):
...
class TestMainFunction:
    ...
    def test_main_text_output_format(self, mock_analyze, monkeypatch, capsys):
        ...
        mock_data = {
            ...
            'line_churn': {1: 5},
            'hot_lines': [
                {'line_number': 1, 'changes': 5, 'churn_score': 5.0}
            ],
            'summary': {
                'total_lines': 10,
                'lines_with_changes': 5,
