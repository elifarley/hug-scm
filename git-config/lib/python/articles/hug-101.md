...
2. **Shorter is safer.** `hug a` stages tracked files only; `hug aa`
   ...
   file's unstaged changes; `hug w wipe-all` wipes staged and unstaged
   changes across the whole repo. The longer you type, the more powerful
   (and dangerous) the command.
...
### 4. When something goes wrong
    ...
    hug h back             # move HEAD back one, keep commit changes staged + preserve all uncommitted work
    hug h undo             # move HEAD back one, put commit changes back unstaged
    hug h rollback         # undo last commit, discard its changes (pre-existing work is preserved)
...
**Discarding work:**
    ...
    hug w discard <file>   # revert one file's unstaged changes (smallest blast radius)
    hug w wipe <file>      # wipe staged + unstaged changes on a file
    hug w wipe-all         # wipe staged + unstaged across all tracked files (nuclear)

Same shape: more letters → more power. Both `wipe` and `wipe-all` confirm
...
- `hug help :agents` — dense imperative cheatsheet for AI coding agents
