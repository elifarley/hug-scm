...
- `shc [<commit>]`: Files changed in commit. Args: `[<commit>]` (optional, default HEAD; also `N`/`-N` and ranges), `[-n|--name-only]` (optional: paths only, one per line), `[-- <path>...]` (optional pathspecs).
