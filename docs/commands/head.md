...
- List just the affected file paths (scriptable, repo-relative): `hug shc -n HEAD~3..HEAD`. For filenames containing newlines/backslashes use `hug shc -n -z` (NUL-separated — pair with `xargs -0 -r` (GNU; BSD/macOS xargs needs no -r)); at most one positional is accepted. Note the default (display) stance lists only a rename's NEW path: consumers that act on every touched path (e.g. `rm`, `restore`) must use `hug shc -n -z --no-renames`, which lists BOTH sides of a rename.
