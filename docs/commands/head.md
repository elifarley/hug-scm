...
- Preview cumulative file changes before squashing: `hug shc HEAD~3..HEAD` shows all files that would be affected by squashing the last 3 commits. Stats output is deterministic under hostile `core.quotePath` / `diff.renames` / `diff.relative` / `diff.ignoreSubmodules` configs: non-ASCII paths print raw (not C-quoted), renames collapse to one `{old => new}` line, and submodules are always shown.
- List just the affected file paths (scriptable, repo-relative): `hug shc -n HEAD~3..HEAD`. For filenames containing newlines/backslashes use `hug shc -n -z` (NUL-separated — pair with `xargs -0 -r` (GNU; BSD/macOS xargs needs no -r)); at most one positional is accepted.
- Merge commits answer "what changed" too: `hug shc <merge>` lists changes against EACH parent (a file touched on both sides appears once per parent; the same per-parent stats flow into `hug sh` / `hug shp` / `hug shcp`). Patch sections on merges show the FIRST-PARENT diff — what the merge brought in (the same rule as `hug dd`; empty when the merge introduces nothing vs parent 1, e.g. `-s ours`) — while stats list changes against EACH parent.
