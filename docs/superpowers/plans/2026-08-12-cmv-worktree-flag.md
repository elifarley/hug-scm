...
```bats
@test "create_worktree_for_branch: creates a new branch + worktree and prints resolved path" {
  local repo
  repo=$(create_test_repo)
  pushd "$repo" >/dev/null

  local path
  path=$(HUG_FORCE=true create_worktree_for_branch feature-x --base HEAD --force)
  assert_success
...
@test "create_worktree_for_branch: reuses existing branch without --base" {
  ...
  local path
  path=$(HUG_FORCE=true create_worktree_for_branch existing-wt --force)
  assert_success
  assert_worktree_exists "$path"
...
```bash
# Recovery: inverse cmv (suppressed under --quiet).
if [[ ${HUG_QUIET:-f} != T ]]; then
