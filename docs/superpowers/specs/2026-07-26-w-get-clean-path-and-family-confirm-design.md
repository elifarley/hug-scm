...
- **Date:** 2026-07-26
- **Status:** Design draft, under user review (revised after code-roast)
...
- **Exit 1** (`HUG_EX_FAIL`) = "blocked by state or declined." This covers `w get` State B (dirty-tree refusal, via `check_working_tree_clean`'s `error` → exit 1), interactive confirmation declines, and missing/invalid args. It is the universal "could not proceed" code across the family (`wtdel`, `rb`, `bdel`, `w-discard` all decline/blocked → exit 1).
...
1. **Add `get_dirty_files`** — non-exiting helper that prints files with staged OR unstaged changes to stdout, one per line, deduped+sorted. **Mirror `check_files_clean`'s exact idiom** (subshell group piped to `sort -u` via process substitution — the canonical form already in this file at lines 99–102), extended to accept an optional file scope and to be non-exiting. With **no file arguments**, it reports the **entire worktree** (needed by the all-files reset path, which destroys edits in files identical between target and HEAD too).
   ...
   # Prints files with staged OR unstaged changes (deduped, sorted). Non-exiting.
...
2. **`reset_specific_files`** (currently checks cleanliness per-file via `check_file_staged` + `check_file_unstaged`, two `git diff` calls per file — O(2n)). Keep the per-file `check_file_in_commit` loop (existence is genuinely per-file) to build `files_to_reset`. Replace the per-file staged/unstaged loop with the state gate:
   ...
   - **State C** (`HUG_FORCE == true`): compute the dirty subset once via `get_dirty_files "${files_to_reset[@]}"` (needed only to name the files in the notice); `warning "Discarding uncommitted changes in M file(s) (will be overwritten):"` + the list → proceed (no prompt; force authorized it). (Specific-files restore only overwrites — no `rm -f` of absent-from-target files here, so "overwritten" is accurate; the "or deleted" wording is all-files-only.)
...
3. **`reset_all_files`** (currently calls `check_files_clean "${affected_files[@]}"` at line 124, which `exit 1`s with the `wipe`/`wipe-all` message). The all-files reset performs `git restore --source=$commit --staged --worktree .` — a **full tracked-worktree + index** reset to the target commit (untracked and ignored files are unaffected, matching `git restore`'s scope). This means a *tracked* file that is identical between target and HEAD (so not in `affected_files`) but has uncommitted local edits **also** has those edits destroyed. Therefore the safety gate must scope to the **entire tracked worktree**, not `affected_files`:
   - **State C** (`HUG_FORCE == true`): compute the dirty set over the whole tree — `mapfile -t dirty < <(get_dirty_files)` (called with **no args** → whole worktree). Emit `Discarding uncommitted changes in M file(s) (will be overwritten or deleted):` + the full list, then proceed. (Computing over `affected_files` here would **undercount** and silently miss files whose edits get destroyed — the State-C notice must name everything at risk. Wording says "overwritten or deleted" because the reset-all operation both overwrites modified files and `rm -f`'s files absent from the target.)
   - **State B** (`HUG_FORCE != true`): replace the `check_files_clean "${affected_files[@]}"` call (which has the same undercount gap) with the whole-tree `check_working_tree_clean` (already in the lib — fires on any staged OR unstaged change across the whole tree, with the correct `wipe`/`wipe-all` remediation per #208). Exit non-zero on any uncommitted change.
   - **State A** (whole tree clean): proceed, no prompt.
   - **The "Already at target commit" early-exit (line 117) MUST move AFTER the State-B gate**, not before it. Today it sits at line 117, ahead of the cleanliness check (line 123), and is computed over `affected_files`. Scenario it breaks: HEAD content == target content (so `affected_files` is empty) but the worktree is **dirty** → the early-exit fires "Already at target — no files to reset" and `return 0`, never reaching the cleanliness gate — silently passing a dirty worktree as a clean no-op (contradicts State B; misleads the user, though it does not destroy edits since it returns before the restore). After the fix, the order is: resolve target → State-B `check_working_tree_clean` (refuse if dirty) → *then* the "Already at target" early-exit (true no-op, clean tree) → State-C force path / proceed. Add a test: HEAD content == target content + dirty worktree + no `-f` → exit ≠0 (refused), NOT "Already at target".
   (State B uses `check_working_tree_clean` for its ready-made `wipe`/`wipe-all` refuse message; State C uses `get_dirty_files` (no args) because it needs the **file list** to print. Both do whole-tree detection — that's intentional, not duplication: one needs a message, the other needs a list.)
...
- `--dry-run` on dirty path (`< /dev/null`) → preview + refuse message + exit ≠0.
...
**`tests/unit/test_working_dir.bats`** (Concern 2):
- The improved `check_file_unstaged` message appears for the single-file dirty case (via a direct caller or `w get` State B multi-file variant).
