...
- **Date:** 2026-07-26
- **Status:** Design draft, under user review (revised after 8 code-roast rounds)
...
|---|---|---|---|---|
| **A — clean** | target files have no staged/unstaged changes | ✅ succeed, **no prompt** | ✅ succeed | ✅ succeed **silently** (no "Files are clean" chatter, no discard notice — see note) |
...
**Note on force + clean (the 4th case):** "clean" is orthogonal to "forced". Under `-f` on a clean set, the command proceeds **silently** — it does NOT print the State-A "Files are clean" info line and does NOT print the State-C discard notice (M=0, notice suppressed per File 2 step 2/3). This matches the current force path, which is entirely silent (all of git-w-get's chatter is `! $force`-gated). Do NOT add "Files are clean" chatter under `-f` — that would regress the silent force path. In §3 the force+clean run is handled by State A's "proceed, no prompt" with chatter suppressed under force; the existing `-f`-on-clean tests are "State C logic with an empty dirty set" only in the sense that the dirty array is empty — behaviorally it is the silent clean path.
...
**`--dry-run`** behaves per state: A → preview + "no changes to lose" success; B → **refuse at the cleanliness gate before any diff preview** (the refuse block names the dirty files on the specific-files path, or shows counts + a `hug sl` pointer on the all-files path — that is the only "preview" relevant when refusing; do NOT print the full diff preview then refuse, which wastes compute on a path that will exit anyway) + exit ≠0; C → preview + "would discard (overwrite or delete)" message, no file changes. (Operational order in `reset_all_files`: the cleanliness check at line 124 fires before the preview block at lines 127+, so under `--dry-run`+dirty the refuse wins — preserve that order.)
...
- `w get` end-to-end still asserts the refusal surfaces a review target — **specific-files** names the dirty files in the message; **all-files** shows staged/unstaged counts + a `hug sl` pointer (does NOT name files). That is the user-visible half of Concern 2 delivered by the gate redesign, tested under the §4 Concern 1 block. Do not assert the all-files refusal names individual files — it doesn't.
...
- **`check_files_clean`/`check_working_tree_clean` remediation text saying "review first"** — Concern 2's half (a). The text is byte-locked (#208) and shared by many commands, so improving it (to add an explicit `hug sw` review-first step, and to make the all-files path name the dirty files like the specific-files path does) is a separate, broader message-quality change. File as a follow-up. `w get`'s refusal surfaces a review target (specific-files names files; all-files shows counts + `hug sl`), which is the actionable half delivered here.
