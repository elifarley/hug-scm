...
| `test_head:592` | rewind non-upstream confirm (`echo "rewind"`) | yes → danger | this test does NOT call `setup_gum_mock`, so the earlier "→ gum-mock `HUG_TEST_GUM_INPUT`" recipe is **unwinnable** (mock not on PATH; real gum TTY-fails; `HUG_TEST_GUM_INPUT` is inert). → `run bash -c 'export HUG_DISABLE_GUM=true; echo "rewind" \| hug h rewind'` (danger's `read` consumes the pipe) |
| `test_head:1034`, `:1039` | rewind non-upstream decline/confirm | yes → danger | same — `HUG_DISABLE_GUM=true` + piped `read`: `echo "not_rewind"` → decline, `echo ""` → decline, `echo "rewind"` → accept (clean 3-run mapping, no mock-counting subtlety) |
| `test_commit:522` | cmv decline (`echo "n"`) | yes → warn | **verify — passes unchanged in CI** (decline via real-gum TTY failure; the `"n"` is never consumed — today's decline comes from the same TTY failure against `gum input`). Optionally harden with `HUG_DISABLE_GUM=true` so the decline exercises warn's `read` deterministically. |
...
| `test_commit:653`, `:664` | cmv creation-prompt decline (`HUG_DISABLE_GUM=true; echo "n"`) | yes → warn | warn declines on `n` (`^[Yy]$` fails) → exit 1 → no change; verify. (Stays green under both tiers — danger `"n" != "move"` → 1, warn `"n"` fails `^[Yy]$` → 1; listed for inventory completeness since the gate's mechanism changes.) |
| `tests/CLAUDE.md:247,251` | doc examples | — | update to match new tiers |

(The `:510`/`:527` rollback tests exercise the **root-commit** branch, which this spec does not touch — an earlier draft wrongly listed them for warn-migration. A round-1 draft also falsely claimed "h-undo/h-squash/h-cmv had no confirmation tests" — they do: `test_head:882/:906/:1291` (gum-mock cancel) and `test_commit:522/:572/:628` (piped). After danger→warn, the four gum-mock-cancel tests break because `HUG_TEST_GUM_INPUT_RETURN_CODE` drives only gum-mock's `input`/`filter`, not `confirm` — and an unset `HUG_TEST_GUM_CONFIRM` auto-confirms (exit 0, see mechanical note), so the command proceeds and `assert_failure` goes red. Migrate them to `HUG_TEST_GUM_CONFIRM=no`.)

**Mechanical note (corrected — there are TWO gum environments; the suite default is real gum, not gum-mock):** `test_helper.bash:96` exports `HUG_TEST_MODE=true` suite-globally, which makes `gum_available` (hug-gum:27) return true — but that only **licenses a `gum` invocation**; it does NOT put gum-mock on PATH. The mock is on PATH **only** inside tests that call `setup_gum_mock` (test_helper:788 prepends `tests/bin`; nothing else does — not `setup()`, not `run-tests.sh`, not the Makefile). So:
- **Default (no `setup_gum_mock`):** a `gum` call resolves to the **real** gum (`~/.hug-deps/bin/gum` via `bin/activate`, installed by `make optional-deps-install`). Under BATS (no TTY) real gum **TTY-fails → exit 1 → decline**, and piped stdin is NOT read. (No gum at all → 127 → same decline path.) So the suite default for a bare `echo "y" | hug h …` is **decline-by-TTY-failure, NOT auto-confirm.** (Earlier drafts claimed the default was gum-mock auto-confirm — that is false; the auto-confirm branch at gum-mock:186–205 is reachable only inside `setup_gum_mock`.)
- **Under `setup_gum_mock`:** gum-mock's `confirm` reads `HUG_TEST_GUM_CONFIRM` (gum-mock:186–188); unset → **exit 0 (silent auto-confirm)**; `=no` → decline. Its `input` subcommand (danger's typed-word gate) reads `HUG_TEST_GUM_INPUT` and `HUG_TEST_GUM_INPUT_RETURN_CODE`.

**Consequences for the migrated tests:**
- The **four warn-cancel tests** (`test_head:720/:882/:906/:1291`) **do** call `setup_gum_mock` (:719/:881/:905/:1290), so they hit gum-mock `confirm`; after danger→warn their `HUG_TEST_GUM_INPUT_RETURN_CODE=1` is inert (drives only `input`/`filter`), and unset `HUG_TEST_GUM_CONFIRM` would auto-confirm → must set `HUG_TEST_GUM_CONFIRM=no`.
- For any test that does **NOT** call `setup_gum_mock`, the deterministic way to drive a prompt is `HUG_DISABLE_GUM=true` + piped `read` (as `:639`/`:644` already do) — this makes `gum_available` false so warn/danger fall through to `read`, which consumes the pipe. Use this for the `h-rewind` danger tests too (see table).
