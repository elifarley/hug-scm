...
>
> **Roast round 5 (2026-07-29, confirmation pass).** Three of the four round-4 rewrites verified correct — including the per-mode `-y` rationale, confirmed **airtight for all four modes** (escalation is structurally impossible for `--back`/`--undo`/`--rollback`; `--rewind`'s predicate provably covers everything `--hard` destroys, including mid-conflict, by sourcing the real `get_dirty_files`). The one remaining Major was in round 4's §8 rewrite: the blanket "assert ss/su equal the pre-op capture" is falsified by a staged file + `h-undo` (a perfect recovery brings it back *unstaged* — mixed reset resets the index, so `write-tree` differs); §8 item 3 now asserts HEAD == pre-op SHA plus the **per-mode** preservation invariant, and names both naive assertions to avoid. Round-4-introduced minors also fixed: the §12 rename count is **11**, not 8; the "force-push suggested" clause is attributed to its real source (the divergence tip at `hug-git-commit:437`, fired because the fabricated commit diverges the branch — `git-h-squash` itself prints no push suggestion); the `check_git_repo` comment no longer mis-cites the CLAUDE.md template; and the `h-undo` root anchor is `96-121`. After this, sign-off ready.
...
| `h-undo` (root path) | **danger** ⚠️ (unchanged) | `git-h-undo:96-121`: same root-commit reasoning as `h-back` |
