...
> **Revision note (2026-07-28, post-review).** An implementation-grounded review replayed every "Verified" claim against the live code and found the original recovery design dead on arrival: re-invoking a mover command to recover forward **no-ops** (the aligned-target short-circuit, Critical §4/§A) and `cmv`'s hint ran on the wrong branch (Critical §6/§B). The fix adopted here: recovery is a **purpose-built primitive `hug h restore`** (§4, §7 Step 4) that never short-circuits forward targets, and `cmv` is **danger** (no complete recovery exists). §5 is restated as two layers, §10 names the existing `get_dirty_files` primitive and adds the forward-target audit class, and the unverifiable "**Verified:**" prose is replaced by tests that execute the actual hug command (§8).
...
## 4. Recovery command — one purpose-built primitive, mode-flagged
...
**Conclusion:** recovery must not round-trip through `handle_standard_operation`'s aligned-target short-circuit. It gets its own primitive.

### 4.2 The primitive: `hug h restore <target> --<mode>`
...
- **Mode is required, never defaulted:** `--soft | --mixed | --keep | --hard` (`${mode:?}`). Recovery is mode-*matched* to the op being undone (§4.3); a default would let a recovery silently pick the wrong mode and destroy preserved work.
- **State-determined like the rest of the family:** `--soft`/`--mixed`/`--keep` preserve work ⇒ **warn** (auto-confirms with `-y`). `--hard` on a tree with uncommitted *tracked* changes ⇒ **danger** (refuses `-y`, exit 3) — discarding those edits is unrecoverable, the same reasoning as dirty-tree `h-rewind`. `--hard` on a clean tree ⇒ warn.
...
The recovery command is uniformly `hug h restore <pre-op-HEAD> --<op-mode> -y`. Each row only supplies the op's reset mode — the mode-match is enforced by construction, not by six hand-built strings:

| Command | Reset mode | Post-op state to preserve | Recovery (run immediately after) | Why it's safe |
|---|---|---|---|---|
| `h-back` | `--soft` | changes **staged** | `hug h restore <pre-op-HEAD> --soft -y` | `--soft` moves HEAD only; staged changes stay staged |
| `h-undo` | `--mixed` | changes **unstaged** | `hug h restore <pre-op-HEAD> --mixed -y` | `--mixed` resets index, leaves working tree untouched |
| `h-rollback` | `--keep` | uncommitted work preserved | `hug h restore <pre-op-HEAD> --keep -y` | forward `--keep` always succeeds when the original ran (§5) |
| `h-rewind` (clean) | `--hard` | nothing (clean) | `hug h restore <pre-op-HEAD> --hard -y` | `--hard` to prior commit; tree was clean, nothing to lose; restore is warn on a clean tree |
| `h-squash` | `--soft`+recommit | (history rewrite) | `hug h restore <pre-op-HEAD> --soft -y` | `--soft` forward restores original commits; index keeps the (byte-identical) squashed tree |
| `cmv` | `--hard` (clean-gated) | (branch move + new SHAs) | **— none; danger tier** | switches branch AND rewrites SHAs; no single same-branch reset recovers "exactly what changed" (§6) |
...
**Flag on the recovery command:** uniformly **`-y`** — `restore` is warn-tier in the recovery context (clean tree at recovery time, except the `--hard`+dirty escalation which is danger by design). This dissolves the earlier draft's `-y`-vs-`-f` contradiction: recovery no longer re-invokes danger-tier `h-rewind`, so there is no per-op flag to disagree about.
...
**Layer 2 — the hug-level precondition (why the primitive is required).** Layer 1 governs `git reset --keep`. For the *hug* recovery to inherit it, the recovery invocation must actually **reach** a `reset --keep`. Re-invoking `hug h rollback <pre-SHA>` does not — it exits at `handle_standard_operation`'s aligned-target short-circuit (Appendix A) before any reset. `hug h restore <pre-SHA> --keep` does reach the reset, because it never short-circuits forward targets (§4.2). **Therefore the §2 bar is met for `h-rollback` *conditional on the restore primitive*:** recovery is a single command, `hug h restore <pre-op-HEAD> --keep -y`, and Layer 1 guarantees it preserves the uncommitted edits and endangers nothing. Without the primitive, this section's original unconditional claim was false.
...
|---|---|---|
| `h-back` | **warn** + hint | `--soft` preserves all work; `restore --soft` restores HEAD completely |
| `h-undo` | **warn** + hint | `--mixed` preserves working tree; `restore --mixed` restores HEAD completely |
| `h-rollback` (normal) | **warn** + hint | `--keep` preserves uncommitted work and refuses to run if a dirty file is in range; forward `restore --keep` always succeeds (§5) |
...
| `h-squash` | **warn** + hint | history rewrite; original commits reflog-recoverable via `restore --soft` forward |
...
| `h-rewind` (clean tree) | **warn** + hint | **CHANGED from #225's danger** — only HEAD moves; `restore --hard` on the (clean) tree is a complete recovery |
| `h-rewind` (dirty tree) | **danger** + partial hint | uncommitted edits destroyed (unrecoverable); the commit part is recoverable via `hug h restore <pre-op-HEAD> --hard -f` — but the destroyed working-tree edits cannot be, and the hint says so explicitly |
...
4. **Every** warn-tier success prints a `hug h restore …` recovery hint; `h-rewind`-dirty prints the commit-recovery part but stays danger.
...
### Step 4 — `hug h restore` + one branch-aware hint helper

**(a) The primitive** (`git-config/bin/git-h-restore`), the inverse-of-a-mover reset:

```bash
# Usage: hug h restore <target> --soft|--mixed|--keep|--hard [-y|-f]
#   mode is REQUIRED (${mode:?}) — recovery must be mode-matched, never defaulted.
#   Warn-tier, EXCEPT --hard on a dirty tracked tree ⇒ danger (refuses -y, exit 3).
...
if [ "$mode" = hard ] && has_uncommitted_tracked_changes; then
  tier=danger   # --hard would destroy uncommitted tracked edits (unrecoverable)
fi
...
It deliberately does **not** call `handle_standard_operation`; its no-op test is exact-SHA equality, not the range-count gate (Appendix A is the regression this prevents).

**(b) The hint helper** — branch-aware, templates the command from its inputs (no dead parameter):

```bash
# Usage: emit_head_recovery_hint <pre-op-HEAD> <mode>
# Prints to stderr. Called AFTER the op succeeds, so the SHA is final.
# Templates the canonical recovery command; callers never hand-build the string.
emit_head_recovery_hint() {
  local pre_op_head="${1:?}" mode="${2:?}"
  printf '\nℹ️  HEAD moved. Recover with:\n    hug h restore %s --%s -y\n' \
    "$pre_op_head" "$mode" >&2
}
```

Each warn-tier command captures `pre_op_head=$(git rev-parse HEAD)` **before** the git op, runs the op, and on success calls `emit_head_recovery_hint "$pre_op_head" "<its reset mode>"`. The helper takes only `<pre-SHA>` and `<mode>` — it has no way to express "you are on a different branch now," which is precisely why **`cmv` never calls it** (its branch switch is what makes recovery incomplete; §6). If a future branch-switching op needs a recovery, the helper grows a branch precondition then — not speculatively.

**Placement invariant:** the hint prints only on the **success** path of the actual git op, never before it. A failed/aborted op prints no hint (nothing to recover). It is suppressed under `HUG_QUIET=T` (human-facing chatter → stderr, same discipline as the preview); `show_help` says so (§8), since agents reading `-h` would otherwise expect a hint from a scripted op.
...
2. **Forward-target regression (the exact failure mode):** set up a state where the recovery target is a *descendant* of current HEAD, run the printed `hug h restore <pre-SHA> --<mode> -y`, and assert HEAD **moved** (exit 0 *and* HEAD ≠ pre-recovery HEAD). Guards against anyone re-routing `restore` through the aligned-target short-circuit.
...
4. For `h-rollback`, assert the §5 invariant empirically through the *hug layer*: when the original rollback *runs* (dirty file outside the range), `hug h restore <pre-op-HEAD> --keep -y` recovers exactly and preserves the uncommitted edits; when a dirty file is *in* the range, the original op aborts (exit non-zero, HEAD unchanged) so no recovery is needed.

**`hug h restore` unit coverage:** exact-SHA target ⇒ "Already at …", exit 0, HEAD unchanged; forward (descendant) target ⇒ moves; `--hard` on dirty tracked tree + `-y` ⇒ refused, exit 3; `--hard` on clean tree + `-y` ⇒ proceeds; missing `--mode` ⇒ usage error (never a silent default).
...
- Clean tree + no flag → proceeds at warn, prints `hug h restore <pre-op-HEAD> --hard -y` hint; `-y` proceeds (was: refused).
...
**Help text (concern #5):** update each command's `show_help` OPTIONS: warn-tier commands document `-y` auto-confirms + that a recovery hint is printed **and that the hint is suppressed under `HUG_QUIET=T`**; `h-rewind` documents the clean/dirty split (clean ⇒ `-y` works; dirty ⇒ `-f` required); `cmv` documents danger-tier confirmation and that no recovery hint exists (recovery is manual, multi-step, branch-aware); `hug h restore --help` documents the required `--mode` and the `--hard`+dirty escalation.
...
- **Clean-tree `h-rewind` becomes warn + recovery hint** — `-y` now proceeds; the hint `hug h restore <pre-op-HEAD> --hard -y` is printed. Justified by §2: a clean tree yields a complete recovery command, so warn is honest.
...
- **One dirty-detection algorithm.** Define `has_uncommitted_tracked_changes` as a thin boolean over the **existing** `get_dirty_files` (`hug-git-state:98`; staged + unstaged **tracked**, untracked excluded per §7 Step 3). Consumers to unify onto it: `h-rewind`'s tier decision, `cmv`'s clean-gate (`git-cmv:130`), and `hug h restore`'s `--hard`+dirty escalation. **No third copy** — no ad-hoc `git diff --quiet` per command. `get_dirty_files` is already documented as "MUST mirror `check_files_clean`'s … one algorithm, two consumers" (`hug-git-state:91-92`); this makes it three consumers, one algorithm.
...
| Recovery command | absent | **one primitive `hug h restore <SHA> --<mode>`** — never short-circuits forward targets; mode required | re-invoking the mover no-ops on forward targets (Appendix A); a purpose-built reset is the only honest recovery, and mode-flagging enforces the mode-match by construction |
...
**In scope:** the `tier` param on `handle_upstream_operation`; per-command tier declarations (state-determined for `h-rewind`, warn for the four movers, **danger for `cmv`**) consumed by both paths; the **`hug h restore` primitive** + `emit_head_recovery_hint` helper + mode-matched hints; `h-rewind` clean/dirty split (partial #225 revert, §9); `has_uncommitted_tracked_changes` over `get_dirty_files` + `cmv`-gate unification; the §10 guard-completeness audit (incl. the forward-target class) as prerequisite; help-text `-y`/`-f`/recovery/`HUG_QUIET` docs; the consistency guard + recovery-correctness + forward-target regression + `restore` unit + migration tests (§8); the `git-h-squash:206/208` dead-conditional cleanup folded into its gate edit.
