...
>
> **Re-roast revision (2026-07-28).** A second adversarial replay confirmed both prior Criticals genuinely resolved, then found two Major spec-text issues — now fixed: (1) the hint helper's `-y` vs §6/§8's `-f`+caveat contradiction, resolved to uniformly `-y` with the destroyed-edits caveat moved to the op's success output (the tree is tracked-clean post-reset, verified); (2) the empty-target guard (`[[ -z "$target" ]] && exit 0`) that the upstream call-site rewrite must preserve — `h-back`/`h-undo`/`h-rollback` lack it today and crash exit 128 on a synced upstream, while `h-squash` does something worse — it fails **silently**: its empty `$target` word-splits into `hug h back`'s `HEAD~1` default (`git-h-squash:252`) and it re-commits the HEAD commit under a fabricated `[squash] 0 commits…` message, exit 0, orphaning the user's commit. Minors also fixed: naming-caveat example corrected to "diverged with upstream ahead" (strictly-behind no-ops); the Step 5 drift claim (help is static text, guarded by the §8 test); `restore`'s narrow bare-numeric target guard (short SHAs resolve normally); the danger typed-word; and a §4.1 precision clause.
>
> **Roast round 3 (2026-07-28).** A third replay found that two round-2 edits were themselves empirically wrong, now corrected: (1) **Critical** — `h-squash -u` on a synced upstream does NOT crash exit 128; it silently orphans the HEAD commit under a fabricated `[squash] 0 commits…` commit, exit 0 (`git-h-squash:252` word-splits the empty target into `hug h back`'s `HEAD~1` default) — so the §8 lockdown assertion is now "exit 0 AND HEAD unchanged AND no new commit," the only test that distinguishes the fix from the bug; (2) the §4.1 clause overclaimed — `has_pending_changes` counts untracked files, so h-rewind re-invocation recovery succeeds *by accident* when untracked files exist (reworded to "state-accidental, not a contract"). Also fixed: the hint helper now honors `HUG_QUIET` (+ a direct test); the `restore` prompt dispatches per-tier (warn/danger take different arg forms); `error_usage` (not the nonexistent `usage_error`); the crash is `git reset ""`, not `rev-parse --short ""`; the §10 "read from the table" residue; the h-back root anchor (87-103); the guard's actionable "4+-char SHA prefix" message; and a Step 5 `-y`/`-f` evaluation-time note. §10 also establishes the two-predicate dirty-detection model: `has_uncommitted_tracked_changes` (tracked-only, for safety/tier) and `has_untracked_or_pending_changes` (renamed from `has_pending_changes`; tracked+untracked, for caa/w-wip), over one shared `git status` capture — and switches `handle_standard_operation:99` to the tracked-only predicate (its "tracked changes will be reset" message currently fires on untracked-only trees).
>
> **Roast round 4 (2026-07-29).** The evidence base is now unfalsifiable — every replayed claim held; the four remaining defects were all in the new design prose, now fixed: (1) `git reset "$mode"` → `git reset --"$mode"` (the canonical listing crashed exit 128 on every real restore); (2) the §7 Step 4b "uniformly `-y`" justification restated **per-mode** — the earlier "tree is tracked-clean after any reset (verified)" was false (`--soft` leaves changes staged, `--mixed` unstaged); the conclusion was right for a different reason; (3) §8's recovery test now asserts **pre-op vs post-recovery** state (HEAD + ss/su + write-tree), since "ss/su identical before and after recovery" fails *because the recovery works*; (4) §10's three incompatible implementation stories collapsed into one — `has_untracked_or_pending_changes` is `has_pending_changes` renamed with body unchanged (the status capture), `has_uncommitted_tracked_changes` is a thin boolean over diff-based `get_dirty_files`, and agreement is "same git state," not "one process launch" (the latter claim was unjustified; `has_untracked_files` didn't exist). The round-3 note's "one shared `git status` capture" phrasing is superseded by (4). Minors: dangling `§8.2` → "§8 Recovery-hint item 2"; §11 "three consumers" → four; `git-h-squash:251-252` → `:252-253`; §4.2 strictly-behind "no-ops" → crash-today / clean-after-guard; the rename surface (lib tests + README) added to §12 scope; `check_git_repo` added to the listing; a mid-conflict `restore --rewind` test named.
...
check_git_repo   # every hug command opens with this (CLAUDE.md template); restore is no exception
...
3. Recovery restores the **pre-op** state (the §2 bar): capture `hug ss`/`hug su` (and `git write-tree`) **before** the op; after running the printed recovery, assert HEAD == pre-op SHA **and** ss/su equal the pre-op capture. Do **not** assert "ss/su identical before and after recovery" — post-op vs post-recovery differ *because the recovery worked* (a complete recovery of a clean-pre-op `h-back` yields a *clean* tree; the staged residue is re-committed, not lost). Per-mode invariant underneath: `restore --back` leaves the index byte-identical (`git write-tree` equal before/after the restore itself); `restore --undo` leaves tracked worktree files byte-identical; `restore --rollback` leaves locally-edited files outside the range byte-identical (§5).
...
The rename makes the untracked-inclusion explicit in the name; the bare "pending changes" is exactly what let `hug-git-upstream:99` (and this spec's first §4.1 wording) conflate the two. The two predicates **agree because they read the same git state, not because they share one process launch** — `get_dirty_files` (diff-based) and the status capture agree in every state including conflicts (verified). The discipline being extended is `get_dirty_files`'s "MUST mirror `check_files_clean` … one algorithm" doc (`hug-git-state:91-92`); the audit asserts no third detection algorithm appears.
...
- **`h-back` / `h-undo` root paths** (`git-h-back:87-103`, `git-h-undo:96-119`): stay danger. A mechanical danger→warn sweep must skip them — at root, recovery is a guaranteed no-op (unborn HEAD ⇒ `rev-list` fails ⇒ `|| echo 0` ⇒ count 0), so there is no complete recovery to license warn.
