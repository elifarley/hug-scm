...
hug shp [commit]      # SHow: with Patch (commit with patch; merge commits show the first-parent patch)
...
hug shcp [N|commit|range] [-- <path>...] # SHow: Cumulative with Patch (diff + stats, optionally filtered by path; single-commit mode shows the first-parent patch on merges)
